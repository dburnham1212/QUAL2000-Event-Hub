...
// ============ Login Page Testing ============
test("Testing if navigating to login page manually displays the login page", async ({
    page,
}) => {
...
test("Testing if an already created user will not logged in with an incorrect password", async ({
    page,
}) => {
    await page.getByRole("textbox", { name: "Email Address" }).click();
    await page
...
test("Testing if an unknown email and existing password rejects user", async ({
    page,
}) => {
    await page.getByRole("textbox", { name: "Email Address" }).click();
    await page
        .getByRole("textbox", { name: "Email Address" })
        .fill("dburnham@gmail.com");
    await page.getByRole("textbox", { name: "Password" }).click();
    await page.getByRole("textbox", { name: "Password" }).fill("123");
    await page.getByRole("button", { name: "Log In" }).click();
    await expect(page.locator("section")).toContainText(
...
test('Testing if "Create New Account" button takes you to the register page', async ({
    page,
}) => {
...
test("Testing if login works with capitalized email", async ({ page }) => {
    await page.getByRole("textbox", { name: "Email Address" }).click();
    await page
        .getByRole("textbox", { name: "Email Address" })
        .fill("DBURNHAM@EXAMPLE.COM");
